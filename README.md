...
| 呼び名 | 入口 | 仕事 | 人間への届き方 |
|---|---|---|---|
| GraphWrangler AI | 右のチャットドロワー | ページ全体を見てグラフを整理する。ステップ分解の下書き、手順書やスクリプトの起草 | その場の会話 |
| Task AI | 各ノードの会話欄 | そのノードについての相談相手 | 会話。人間にしか決められないことに当たったときだけ判断リクエスト（=あなたの番）を開く |
| 実行AI | 実行エンジン（担当=AI のノードに順番が来ると起動） | ノードを実際に実行し、結果をスレッドに書く | 質問は判断リクエスト。手順書に投稿先が書いてあれば Discord の指定チャンネルへ報告（`discord_post`） |

3人とも実体は同じで、**サーバと同じマシンで claude CLI を都度起動する使い捨てプロセス**。
1つの仕事（1返信・1ノード実行）が終わったら消える。常駐しない。文脈も持ち越さない——
知っているのは、そのとき渡されたノード・ラン・スレッドの中身だけ。
...
| 系統 | いつ | どこへ | 何が自動で付くか |
|---|---|---|---|
| **グラフ通知**（あなたの番） | ボールが人間に渡った瞬間。判断リクエスト・AIの質問・実行前承認・失敗リカバリ・分岐・ランの人間待ち | 固定の通知チャンネル（⚙の Webhook） | 関係者へのメンション・ノードURL |
| **業務連絡** | ノードの仕事そのものが「誰かに連絡する」であるとき | **手順書に書かれたチャンネルだけ**（例:「まとまったら #報告 に投稿」） | 出し元の `[Graph Wrangler]`・関係者メンション・ノードURL・スレッドへの記録 |

前者はノードの進捗から機械的に決まり、後者は手順書が宛先を決める。手順書に投稿先が
書かれていないノードは Discord に何も出さない——「AIが良かれと思って鳴らす」を
仕組みで塞いである（通知が増えるほど、人はそのチャンネルを信用しなくなるため）。

### 常駐AIアシスタントと一緒に使うとき（連携機能ではない）

同じマシンで Discord 常駐のAIアシスタント（OpenClaw など）を飼っている環境がある。
先に誤解を解いておくと、**GraphWrangler にそれらとの連携機能は無いし、要らない**。
互いを呼ばず、依存もしない。住み分けは一行で言える——

> **グラフに関わる仕事は GW の AI が中で完結する。グラフの外の受付・雑用が常駐アシスタント。**

接点はただひとつ、Discord の Bot（トークン）を共有**できる**こと。共有しても両者は
無関係に動き、GW からの投稿は本文先頭の `[Graph Wrangler]` で見分けられる。

やりがちな混線がひとつある。ノードの手順書に「常駐アシスタントの CLI で Discord に
投げろ」と書くことだ。投稿はできてしまうが、GW を通らないのでノードURL・関係者
メンション・スレッドへの記録が付かず、後から「いつ何をどこへ報告したか」が辿れなくなる。
手順書に書くのは**チャンネル名だけ**。送信は GW（`discord_post`）に任せる。
