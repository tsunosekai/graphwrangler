...
/** 担当を script へ上げるときの確認文（試走ゲート。docs/design.md 3.5.1）。
 *  パネルの担当セレクトと、グラフの右クリック「担当を変える」で同じ文を出す
 *  ——同じ操作なのに面によって文言が違う、が起きないように1箇所で持つ */
export const TRIAL_CONFIRM_MESSAGE =
  "スクリプトのテスト実行が成功していません。このまま続けますか？";
...
export const HINT_TEXT = {
  ...
  kind: "実行=ふつうの作業。判断=完了時に枝をひとつ選び、選ばれなかった枝の先はスキップされる。トリガー=開始するとラン（実行インスタンス）が生まれる起点",
  impl: "やり方の中身。担当=人間なら読む手順書、担当=AIなら実行時プロンプトに渡る手順書、担当=スクリプトなら実行コマンド。担当と実装の種類が合っていないと実行に使われません",
  fixed:
    "ロック=やり方の確定。ロック済みのノードはAIが実装を書き換えず、UI上の編集もできない。クリックで切り替え",
  approval:
    "実行の直前に人間の承認ゲートを通す（外部公開・送信・削除など取り返しのつかない操作向け）。トリガーでは自動開始の直前に挟まる（手動の▶はそのまま開始）",
  autonomy:
  ...
  fire: "今すぐ開始してラン（実行インスタンス）を生む。同じルーティーンは並列で回せる（ランの名前で区別）",
  pageProject:
    "トリガーを持たないページ。やりたいことに向かって一度きり進める。ノードにトリガーを置くと自動でルーティーンに変わる",
  pageRoutine:
    "トリガーを持つページ。開始のたびにラン（実行インスタンス）が生まれ、進捗はランごとに付く（実行一覧で見られる）",
  statusLegend:
