...
- **並列ラン（パラレルワールド）**: 同じルーティーンを複数のランで並行して回せる
  ...
  手動▶の発火時にランへ名前（作品名など）を付けて世界線を区別する。
  **ランはテンプレートとは別のページ**（2026-08-08 本人指定「ランを押すとその瞬間に
  ...
  ランのページが見せるもの:
    - グラフとノードの中身 = **発火時点のフォーク**（run.snapshot、無ければ操作ログ復元。5.5）
  ...
  同じ理由でトリガーの ▶ も無効（そのランは発火済み）。新しく始めるのはテンプレートの
  ページか台帳の発火ボタンから。ページを開いた既定はテンプレート（設計図）。
...
- **終いにしたページのトリガーは発火しない**（2026-08-09 本人報告の不具合の修正。
  ...
  アーカイブから戻せば（status=pending）また発火する
...
- **左レール**: ページ一覧を「プロジェクト」（トリガー無し）と「ルーティーン」（トリガー有り）の
  ...
  出ない行き止まりになっていた（かつエンジンは所属ページの status を見ないので裏で発火し
...
1. **焼く**: 発火時に `run.snapshot` へページ構成の中身を写す（このとき以降のランで最も確か）
2. **再生する**: それが無い過去のランは、操作ログ（ops.jsonl）を発火時刻まで再生して復元する
