...
- **並列ラン（パラレルワールド）**: 同じルーティーンを複数のランで並行して回せる
  ...
  **自動発火（script/AI トリガー）も実行中ランの有無に関係なく定刻で発火する**
  （2026-08-08 変更。旧仕様は「実行中ランがあると発火しない」＝積み残し防止だったが、
...
- **左レール**: ページ一覧を「プロジェクト」（トリガー無し）と「ルーティーン」（トリガー有り）の
  ...
  「アーカイブ」節へ畳む。タイトル下の**ちょぼ**はページ内ノードの内訳（ノード1つ=点1つ）。
  ...
  そのランをグラフに投影）が持つ。幅はドラッグでリサイズでき、レール自体も畳める。
