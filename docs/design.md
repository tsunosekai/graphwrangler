...
- **人に関する3軸**は役割が違うので混ぜない:
  ...
  - **assignee** = 単数の責任（担当者）。executor=human のノードで「人間の誰がやるか」。
    ...
    Fix の保護対象外
  - **members** = ページの関係者（宛先集合）。ページ（kind=goal / メンバー持ち）でのみ
    意味を持ち、左レールの人フィルタと「自分の関係分」の判定に使う。作成者は自動で
    関係者扱いなので重複して入れない
- **「あなたの番」の per-user 化**: 従来の「pendingRequest があれば橙」を、
  ...
  騒がないこと）
- **左レールの人フィルタ**: ロスター（GET /api/users）から人を選ぶと、その人の
  「関連ページ」に絞る。関連の定義 = その人が members に居る / createdBy である /
  ページ内のノードの assignee であるページ
...
```jsonc
{
  ...
  "members": [],                  // ページの関係者メール配列（kind=goal / メンバー持ちでのみ意味を持つ。3.11）
  "created": "…"
}
```
