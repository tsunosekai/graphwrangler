...
- **並列ラン（パラレルワールド）**: 同じルーティーンを複数のランで並行して回せる
  ...
  ツールバーのセレクトか左レールのラン子行でどのランを見るか切り替える。
  **既定はテンプレート表示（投影なし）**（2026-08-08 本人指定。旧: 実行中ランがあれば
  勝手に最新1本を投影していたため、素の設計図を見るのに毎回セレクタ操作が要った）。
  ラン表示中はトリガーの ▶ を無効にする——そのランは発火済みで、そこから作れるのは
  常に別のランだから。新しく始めるのはテンプレート表示か台帳の発火ボタンから。
...
- ランの進捗は、選んだラン（左レールのラン子行 / ツールバーのセレクト / URL の `?run=`）を
  グラフ・ノードパネルへ投影する。何も選んでいなければテンプレート表示。
  ラン中のワークアイテムの通知リンクは `?run=` 付きで送るので、通知から踏むと
  そのランの進捗（＝あなたの番の回答導線）へ着地する
