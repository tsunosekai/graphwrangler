...
- **「あなたの番」の per-user 化**: 従来の「pendingRequest があれば橙」を、
  ...
  騒がないこと）
- **左レールの人フィルタ**: ロスター（GET /api/users）から人を選ぶと、その人が
  実効関係者に居るページに絞る。**実効関係者が空のページ（帰属が誰にも記録されて
  いない）はフィルタ中も常に表示する**——チーム化前の既存データは全ノードが帰属無しで、
  隠すと移行期に全ページが消える（2026-08-04 実機で発生）。assignee=null が全員宛で
  あるのと同じ思想で「帰属不明 = 全員のもの」
