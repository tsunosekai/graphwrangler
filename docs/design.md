...
- **並列ラン（パラレルワールド）**: 同じルーティーンを複数のランで並行して回せる
  ...
  **ランはテンプレートとは別のページ**（2026-08-08 本人指定「ランを押すとその瞬間に
  ...
  ランのページが見せるもの:
    - グラフとノードの中身 = **ランを作った時点のフォーク**（run.snapshot、無ければ操作ログ復元。5.5）
...
- ランを開くと、そのランのページ（`#/r/<runId>`）へ移る。テンプレートのページとは
  別物として表示され、混ざらない。ラン中のワークアイテムの通知リンクは `?run=` 付きで
  送るので、通知から踏むとそのランのページの当該ノードへ着地する（＝あなたの番の回答導線）
...
```jsonc
{
  ...
  "snapshot": {                   // ランを作った時点のページ構成（2026-08-08）。null = この機能より前のラン
    "capturedAt": "…",
...
1. **焼く**: ランを作った時点に `run.snapshot` へページ構成の中身を写す（このとき以降のランで最も確か）
2. **再生する**: それが無い過去のランは、操作ログ（ops.jsonl）をランを作った時刻まで再生して復元する
   （`GraphStore.nodesAt`。ts 昇順で並べ替えてから畳む）
