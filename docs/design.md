...
### 3.5 Fix ライフサイクル（会話→文書→スクリプトの3段階。旧称: 硬化）

- Fixは2段でなく**3段**: **会話（判例）→ 手順書=文書（AIが読んで実行）→ スクリプト（決定的）**。
...
- UIではスクリプト化済み/LLM処理中が一目で分かるようにする（バッジ・Fix率）
- **決定的ノードかどうかの操作的テスト: 「前提知識を渡しても結果は変わらないか」**
  （変わらない=決定的=スクリプト化候補。ARK経理ポータルの突合工程の定義を輸入）
- **Fix には対象が2つある（2026-07-31 整理）**: ①**実装の Fix**（ノード単位。impl の
  会話→文書→スクリプト）と ②**形の Fix**（流れ単位。一回きりの段取り→再利用できる
  決まった流れ = ルーティーンページへの昇格）。直交する: ルーティーンの中に会話段の
  ステップがあってよいし、プロジェクトのノードにスクリプト実装があってよい。
  層の違い（プロジェクト/ルーティーン）は②だけを指す
- **ロック/アンロック（selfImprove）は Fix と直交する第3の軸**: Fix が実装の「現在地」、
  ロックは「AI がそれを書き換えてよいか」という権限。アンロック×会話段=AIが自分で
  手順書を書いて Fix を進めてよい（自己Fix）、アンロック×スクリプト段=失敗したら
  AI が直してよい（自己修復）。ロック=人間だけが Fix を進める
- **ノードのFixは現場で自然に起きるが、エッジの自動化はツールがないと起きない**
...
### 3.8 ページの二層: プロジェクトとルーティーン（定期タスク対応。旧称: 手順）

参考実物: ARK経理ポータル /system（同僚AI作。月次経理=Fixしきった手順グラフの実例。
制御フローのグラフに個々の取引=ワークアイテムが繰り返し流れる）。2026-07-29 壁打ちで方向合意。

グラフには意味の違う二層があり、**ページ（グループ）に種類を持たせて**区別する:

| | プロジェクトページ（現行） | ルーティーンページ（将来） |
|---|---|---|
...
- **トレース**: ルーティーンページではラン単位で「何が起きたか」を後から再生できるようにする
  （ARKの実例トレース。定期タスクは回数が積むので必須）。スレッドメッセージに
  ランIDを持たせれば、既存の ops/スレッド形式のまま実現できる
- **Fixの再定義**: プロジェクトで繰り返し現れたパターンをルーティーンページへ昇格させることが
  「Fix」。3.5 のスクリプト化はそのノード単位の姿
...
- 検討中（確度低）: 人間の確定を「今後もこうする」として手順付属の判例集へ昇格させる
  ...
  具体化はルーティーンページの実装時に判断
- AIの発言に**出典バッジ**（どの文書・前提を読んで判断したか）を付けられるようにする。
  スレッドメッセージの payload に参照を持たせて表示（ルーティーンページに限らず有用）
- **台帳ビュー = 第3の投影**: ルーティーンページには「グラフ（工程の地図）」に加えて
...
### 3.9 分岐ノード（条件分岐。2026-07-31 壁打ちで決定）
...
- [x] A/B改善（2026-07-29 第2弾）: 初回セットアップ画面（AI設定: チャット=API
      ...
      チャット→draft提案→承認バーで確定（A-1）/ Houdini風ロック（selfImprove
