...
- **autonomy**: `high` / `normal` / `low` — AI がどこまで人間に聞かずに進むか
  ...
  `OPTION: 選択肢` 最大2行）だけを出力して止まる」規約が入り、エンジンが
  この出力を検知すると done にせず判断リクエストへ変換する（選択肢は id `ai:N`、
  末尾に必ず「飛ばして続ける」= 予約id `skip_continue` と「打ち切る」= `abort_run`。
  3.9b）。回答（選択肢・自由文どちらでも）は
...
- **連鎖規則の範囲は「負けた枝から辿れるノード」に限定する**（2026-09-09 修正）: 「全ての親が
  skipped なら自分も skipped」を、少なくとも1つの親がこの決着由来（直接規則で skipped に
  なったもの、またはそこから連鎖したもの）のノードだけに掛ける。限定が無いと、3.9b の
  手動スキップ（飛ばして先へ進む）やラン作成時の「未計画テンプレートは skipped」の下流が、
  同じページの無関係な分岐を決めた瞬間に「親が全部 skipped」に該当して止まっていた。
  選び直し側（collectSkipsDerivedFrom）が最初からこの範囲だったので、決着側を揃えた

### 3.9b 飛ばす・ここで打ち切る（2026-09-09）

「このノードはやらないで先へ進む」と「ここでもう続ける意味がない」は、それまで人間が
普通に押せる形では無かった（パネルの進捗は プラン済み→着手→完了 と戻す だけ。dropped は
AI の質問カードの「中止」、skipped は分岐の負けた枝でしか付かず、ノード削除で代用すると
ルーティーンではテンプレートが変わってしまう）。2つの操作として足した。

- **飛ばす（skip）**: status を **skipped** にするだけ。下流へは伝搬しない——frontier は
  「skipped でない親が全て done」なので、飛ばした親は無かったことになり下流はそのまま着火する。
  分岐の負けた枝と同じ status だが意味は逆（あちらは「その枝は通らない」で下流も止まる）。
  連鎖規則が巻き込まないように 3.9 の範囲限定がある。frontier は要求しない（まだ順番が
  来ていないノードを先に「やらない」と決めるのは計画の操作で、実行フェーズの原則の対象外）。
  対象外: トリガー（進捗を持たない）・**分岐**（choice が無いまま全ての枝の子が frontier に
  乗ってしまう。決着経路は「分岐を選ぶ」だけ）・決着済み・進行中（先に「戻す」）。
  open な判断リクエストがあれば閉じる。取り消しは「戻す」（status=pending）
  - プロジェクト: パネル「飛ばす」→ `POST /api/nodes/:id/skip`（GraphStore.skipNode）
  - ルーティーン: **テンプレートには出さない**（ランのアイテムはテンプレートの status を見ずに
    pending で作られるので、テンプレートを skipped にしても次回に何の効果も無い）。ラン投影・
    台帳の「このランでは飛ばす」（アイテム status=skipped。以前は AI/script の失敗時だけ
    出していた救済導線を常時に広げた）
- **ここで打ち切る（abort）**: このノードを **dropped**（done なら done のまま＝「ここまで
  やって打ち切る」）にし、**下流（子孫）の未決着を skipped** にし、**器を閉じる**——
  プロジェクトならページを dropped（アーカイブ節へ。isClosedPage でトリガーも止まる）、
  ルーティーンの回なら**ランを cancelled**（テンプレートは触らない＝次のランは普通に作られる。
  「もう回さない」はページのアーカイブで表す）。applyDecision と同じ patch の連続で、
  Ctrl+Z は1手ずつ戻る。**戻すときの範囲は人間が判断する**——起点を「戻す」で復帰しても
  下流の skipped とページ/ランの閉じは自動では戻さない（revertDecision の「下流の done/dropped は
  戻さない」と同じ原則）
  - プロジェクト: パネル「打ち切る」（確認ダイアログ）→ `POST /api/nodes/:id/abort`
    （GraphStore.abortFrom。返り値 {node, skipped, page}）。MCP は node_abort
  - ラン: ラン投影の「打ち切る」・台帳の丸の右クリック「ここで打ち切る」→
    `POST /api/runs/:id/abort {nodeId}`（RunStore.abortFrom）。MCP は run_abort。
    既存の「ランを打ち切る」（`/cancel`）も同時に揃え、**未決着のアイテムを skipped にする**
    ようにした（以前は run.status だけ cancelled にしてアイテムは pending のまま残していた）
- **AI 質問カード（QUESTION 規約）の末尾は「飛ばして続ける」「打ち切る」の2択**
  （予約 id `skip_continue` / `abort_run`。AI 提示の選択肢は最大2個に減らしてスキーマの
  max 4 に収める）。以前の「中止」（このステップだけ dropped、予約 id `abort`）は下流が
  永久に止まる行き止まりで残す価値が薄かったため置き換えた。旧 id の回答は互換のため
  引き続き dropped として解釈する。失敗リカバリカードの「中止」（`abort`）と不可逆ゲートの
  「やめる」（`skip`）は従来どおり dropped（これらは「このステップを諦める」で意味が違う）
...
進行中・完了・中止には「戻す」（待ちへ）。一本道から降りる操作は **「飛ばす」（skipped＝
やらずに先へ）と「打ち切る」（dropped＋下流を見送り＋器を閉じる）** の2つ（3.9b）。
...
```jsonc
{
  ...
  "status": "running",            // running / done / cancelled（cancelled のとき未決着だった items は skipped。3.9b）
